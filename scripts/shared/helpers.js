...
async function getFrameSigner(options) {
  try {
    // Initialize a JSON-RPC provider with the correct URL
    const frame = new ethers.providers.JsonRpcProvider("https://bsc-testnet.publicnode.com");
    
    let networkToCheck = network; // Assuming network is defined elsewhere

    // Get the signer from the frame
    ...
    // Get the chain ID from the signer and compare it to the network's chain ID
    if (await getChainId(networkToCheck) !== await signer.getChainId()) {
